...
export default function AdminEventsPage() {
  ...
  const mockEvents: Event[] = [
    {
      ...
      description: 'Annual startup pitch competition where entrepreneurs present their innovative ideas to a panel of investors and industry experts.',
      event_date: '2024-02-15',
      event_time: '14:00',
      location: 'Startup Kano Main Hall',
      max_participants: 50,
      ...
      registrations: [
        ...
        },
        {
          ...
          event_id: '1',
          user_id: 'user2',
          registered_at: '2024-01-21T14:30:00Z',
          status: 'registered',
          user: { full_name: 'Jane Smith', email: 'jane@startupkano.com' }
        }
      ]
    },
    {
      ...
      description: 'Hands-on workshop covering the latest trends in technology innovation, including AI, blockchain, and IoT applications.',
      ...
      registrations: [
        {
          ...
          event_id: '2',
          user_id: 'user3',
          registered_at: '2024-01-25T11:00:00Z',
          status: 'registered',
          user: { full_name: 'Mike Johnson', email: 'mike@startupkano.com' }
        }
      ]
    },
    {
      ...
      description: 'Monthly networking event for entrepreneurs, investors, and industry professionals to connect and share ideas.',
  ...
  const handleCreateEvent = async () => {
    try {
      if (isOffline) {
        toast.error('Cannot create events in offline mode')
        return
      }
  ...
  const handleUpdateEvent = async () => {
    ...
    try {
      if (isOffline) {
        toast.error('Cannot update events in offline mode')
        return
      }
  ...
  const handleDeleteEvent = async (eventId: string) => {
    ...
    try {
      if (isOffline) {
        toast.error('Cannot delete events in offline mode')
        return
      }
  ...
  const handleUpdateEventStatus = async (eventId: string, status: Event['status']) => {
    try {
      if (isOffline) {
        toast.error('Cannot update event status in offline mode')
        return
      }

      const result = await apiClient.updateEventStatus(eventId, status)
