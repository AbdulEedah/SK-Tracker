...
export default function AdminMeetingsPage() {
  ...
  const mockMeetings: Meeting[] = [
    ...
    },
    {
      ...
      description: 'Final preparation session for the upcoming investor pitch presentation.',
  ...
  const handleCreateMeeting = async () => {
    try {
      if (isOffline) {
        toast.error('Cannot create meetings in offline mode')
        return
      }
  ...
  const handleUpdateMeeting = async () => {
    ...
    try {
      if (isOffline) {
        toast.error('Cannot update meetings in offline mode')
        return
      }
  ...
  const handleDeleteMeeting = async (meetingId: string) => {
    ...
    try {
      if (isOffline) {
        toast.error('Cannot delete meetings in offline mode')
        return
      }
  ...
  const handleUpdateMeetingStatus = async (meetingId: string, status: Meeting['status']) => {
    try {
      if (isOffline) {
        toast.error('Cannot update meeting status in offline mode')
        return
      }

      const result = await apiClient.updateMeetingStatus(meetingId, status)
